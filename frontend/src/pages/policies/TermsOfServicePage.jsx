...
export default function TermsOfServicePage() {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try { await navigator.share({ title: 'Terms of Service', url: window.location.href }) } catch (err) {}
    } else {
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            >
              ...
              <p className="text-[14px] text-white/70 leading-relaxed">
                By using this app you agree to our <button onClick={() => navigate('/privacy')} className="text-[#0A84FF] hover:underline">Privacy Policy</button> and <button onClick={() => window.scrollTo({top:0, behavior:'smooth'})} className="text-[#0A84FF] hover:underline">Terms of Service</button>.
              </p>
            </motion.div>
          </div>
        </div>
